...
export default function Contact() {
  ...
  return (
    <>
      <Helmet>
        <title>Contact — Tanishk Logistics</title>
        <meta name="description" content="Get in touch with Tanishk Logistics for transport contracting, freight coordination and container handling services in Gujarat." />
        <meta property="og:title" content="Contact Tanishk Logistics" />
        <meta property="og:description" content="Phone, email and address for Tanishk Logistics — Galpadar, Gujarat." />
      </Helmet>
      ...
      <section className="container-x py-16 grid gap-10 lg:grid-cols-5">
        <div className="lg:col-span-2 space-y-4">
          ...
          <InfoCard icon={Phone} label="Phone" value="+91 98250 31792" href="tel:+919825031792" />
          <InfoCard icon={Mail} label="Email" value="tanishklogistics9@gmail.com" href="mailto:tanishklogistics9@gmail.com" />
          <InfoCard icon={MapPin} label="Address" value="Survey No. 29/2, Near Renault Showroom, National Highway No. 8/A, Galpadar - 370240, Gujarat, India" />
          <a
        ...
        <form onSubmit={onSubmit} className="lg:col-span-3 rounded-2xl border border-border bg-card p-7 md:p-9 shadow-[var(--shadow-elegant)]">
          ...
          <div className="mt-6 grid gap-4">
            <Field label="Your name">
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} maxLength={100} className="input" placeholder="Full name" />
            </Field>
            <Field label="Phone number">
              <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} maxLength={20} className="input" placeholder="+91 ..." />
            </Field>
            <Field label="Message">
              <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} maxLength={1000} rows={5} className="input resize-none" placeholder="Briefly describe your freight or container requirement" />
            </Field>

            {error && <p className="text-sm text-destructive">{error}</p>}
            {sent && <p className="text-sm text-orange-brand font-semibold">Opening WhatsApp with your message…</p>}

            <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-md bg-gradient-accent px-6 py-3 text-sm font-semibold text-navy-deep shadow-[var(--shadow-glow)] hover:brightness-105 transition">
